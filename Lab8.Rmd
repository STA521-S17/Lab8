...
data = list(Y = sqrt(College$Apps),   # variance stabilizing transformation
            X=cbind(scaled.X), p=ncol(X))
data$n = length(data$Y)
data$scales = c(1, attr(scaled.X, "scaled:scale"))
...
```{r}
parameters = c("beta.s", "beta", "beta0", "lambda.beta", "sigma")
```
...
plot(best.rr$coef, beta.s, xlab="Ridge Regression", ylab="Bayes Posterior Mean (scaled)")
abline(0,1)
```
